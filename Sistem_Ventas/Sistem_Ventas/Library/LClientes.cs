    ...
    public List<TTicket> getTickets (String valor)
        {
            ...
            if (valor == null)
            {
                listTickets = _context.TTickets.Where(u=>u.Propietario.Equals("Cliente")).ToList();
            }
            else
            {
                listTickets = _context.TTickets.Where(u => u.Email.StartsWith(valor) &&  u.Propietario.Equals("Cliente")).ToList();
            }
            List<TTicket> lists = new List<TTicket>();
            listTickets.ForEach(item =>
            {
                lists.Add(new TTicket
                {
                    ID=item.ID,
                    Deuda=item.Deuda,
